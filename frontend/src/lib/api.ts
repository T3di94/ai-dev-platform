...
export type RuntimeConfig = { agent: string; runtime: RuntimeMode; model?: string; configured: boolean; reason?: string };
...
export type PlanStep = { key: string; title: string; agent: string; runtime: RuntimeMode; dependsOn: string[]; taskId?: number };
export type Orchestration = { id: string; title: string; status: TaskStatus; steps: PlanStep[]; createdAt: string; updatedAt: string; error?: string };
...
export const api = {
  ...
  createOrchestration: (title: string, runtime: RuntimeMode) => request<Orchestration>("/orchestrations", { method: "POST", body: JSON.stringify({ title, runtime }) }),
  executeOrchestration: (id: string) => request<Orchestration>(`/orchestrations/${id}/execute`, { method: "POST" }),
};
