...
export default function HomePage() {
  ...
  async function runTask(id: number) {
    try {
      const updated = await api.updateTask(id, "In progress");
      setTasks((current) => current.map((item) => item.id === id ? updated : item));
      setMessage("Task execution started.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to start task");
    }
  }
  ...
  return (
    <main className="shell">
      <header className="topbar">
        <div><div className="eyebrow">AI DEV PLATFORM</div><h1>Agent Workspace</h1></div>
        <div className="status-pill"><span /> {message}</div>
      </header>
      <section className="hero">
        <div><p className="eyebrow">ORCHESTRATED DEVELOPMENT</p><h2>Ship software with specialized AI agents.</h2><p className="muted">Create work, route it to the right agent, monitor execution, and keep quality gates visible in one workspace.</p></div>
        <div className="stats"><div><strong>{counts.total}</strong><span>Total tasks</span></div><div><strong>{counts.active}</strong><span>Running</span></div><div><strong>{counts.ready}</strong><span>Ready</span></div></div>
      </section>
      <section className="agents-grid">
        {agents.map((agent) => <article className="agent-card" key={agent.name}><div className="agent-head"><span className="agent-dot" /><span className="agent-status">ONLINE</span></div><h3>{agent.name}</h3><p>{agent.role}</p><div className="progress"><span /></div><small>Ready for assigned work</small></article>)}
      </section>
